...
try {
    ...
    if (-not (Test-Path $ssmsExe)) {
        throw "SSMS not found at '$ssmsExe'. Pass -SsmsRoot pointing at your SSMS 18 install folder."
    }

    $vsix = Get-ChildItem -Path $PSScriptRoot -Filter 'RightWaySqlFormatter.SSMS18.vsix' | Select-Object -First 1
    if (-not $vsix) { throw "RightWaySqlFormatter.SSMS18.vsix not found next to this script." }

    $ssmsProcs = Get-Process Ssms -ErrorAction SilentlyContinue
    if ($ssmsProcs) {
        throw "SSMS is running (PID $($ssmsProcs.Id -join ', ')). Close ALL SSMS windows - every version, SSMS 18 and 22 share the process name - and re-run."
    }
    ...
    Write-Host ""
    Write-Host "SUCCESS. Start SSMS 18 - 'Format T-SQL Code (Right Way)' appears in the Tools menu (Ctrl+K, F)." -ForegroundColor Green
}
catch {
    Write-Host ""
