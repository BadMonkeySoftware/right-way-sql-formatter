...
Expected output files live in `RightWaySqlFormatter.Tests/Data/StandardFormatSql/` and `ParsedSql/`. These files define what correct formatter behavior looks like. Changing them to make tests pass is how bugs get hidden.
...
- When expected files legitimately need updating (e.g. a new feature adds new expected output), use the `ExpectedOutputRegenerator` explicit test (`REGEN_FILES="file1.sql;file2__SlugName.sql" dotnet test RightWaySqlFormatter.NoSSMS.slnx --filter "Name~RegenerateExpectedFiles"`) — it drives the formatter library directly. Do not hand-write expected output; hand-written expected files have been wrong before. Option slugs (the `__Slug1_Slug2` filename suffix) are defined in `RightWaySqlFormatter.Tests/Utils.cs` (`CONFIG_SLUGS`).
