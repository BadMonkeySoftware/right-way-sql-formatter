...
WHERE d.recovery_model IN (
        1
        , 2
        ) AND b.type IS NULL AND d.database_id NOT IN (
        2
        , 3
        )
...
WHERE d.recovery_model IN (
        ...
        2
        , 3
        ) AND f.type = 1 -- only show log files
...
AS (
    SELECT ROW_NUMBER() OVER (
            ...
            , DBName
            , LastCleanDBCCDate ORDER BY LastCleanDBCCDate
            ) RowID
    FROM #DBCCResults
    )
DELETE
...
SELECT ServerName
    , DBName
    , CASE LastCleanDBCCDate WHEN '1900-01-01 00:00:00.000' THEN 'Never ran DBCC CHECKDB' ELSE CAST(LastCleanDBCCDate AS VARCHAR) END AS LastCleanDBCCDate
FROM #DBCCResults
ORDER BY 3
...
EXECUTE sys.xp_sqlagent_enum_jobs @is_sysadmin
    , @job_owner
...
*/
EXEC msdb.dbo.sp_send_dbmail @recipients = 'email@domain.com'
    , @body = @@SERVERNAME
...
EXEC msdb.dbo.sp_add_notification @alert_name = N'Severity 016'
    , @operator_name = N'The Database Administrator'
...
*/
SELECT *
    , (wait_time_ms - signal_wait_time_ms) AS real_wait_time_ms
...
CROSS APPLY (
    ...
    WHERE i.OBJECT_ID = i1.OBJECT_ID AND i.index_id > i1.index_id AND i.type_desc <> 'xml'
    ORDER BY i.index_id
    ) AS i2
CROSS APPLY (
    SELECT MIN(a.index_id) AS ind1
    ...
    FULL OUTER JOIN (
        ...
        ) AS b ON a.index_column_id = b.index_column_id AND a.column_id = b.column_id AND a.key_ordinal = b.key_ordinal
    HAVING COUNT(CASE WHEN a.index_id IS NULL THEN 1 END) = 0 AND COUNT(CASE WHEN b.index_id IS NULL THEN 1 END) = 0 AND COUNT(a.index_id) = COUNT(b.index_id)
    ) AS p
...
INNER JOIN sys.dm_db_index_physical_stats(DB_ID(), NULL, NULL, NULL, N'Limited') ps ON db.database_id = ps.database_id
INNER JOIN sys.objects obj ON ps.OBJECT_ID = obj.OBJECT_ID
WHERE ps.index_id > 0 AND ps.page_count > 100 AND ps.avg_fragmentation_in_percent > 30
