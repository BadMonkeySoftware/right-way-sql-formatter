...
INNER JOIN @TestTable [3] ON [2].TestColumn2 = [3].TestColumn2
    AND 1 < 2 ON [1].TestColumn1 = [2].TestColumn1

SELECT [1].*
    ,[2].*
FROM @TestTable [1]
LEFT JOIN (
    @TestTable [2] INNER JOIN @TestTable [3] ON [2].TestColumn2 = [3].TestColumn2
        AND 1 < 2
...
INNER JOIN dbo.D3 T3 ON (T3.k = T2.k) ON (T2.k = T1.k)
