...
namespace PoorMansTSqlFormatterLib.Parsers
{
    public class TSqlStandardParser : ISqlTokenParser
    {
        ...
        // Equivalent to: ^DECLARE [\p{L}0-9_\$\@\#]+ ((INSENSITIVE|SCROLL) ){0,2}CURSOR
        // The expanded unicode-letter ranges are a historical artifact of the upstream
        // project's C#-to-JS transpilation (which couldn't handle \p{L}); they remain
        // correct for .NET and are kept as-is to avoid behavior churn.
                ...
                switch (token.Type)
                {
                    ...
                    case SqlTokenType.OtherNode:
                        ...
                        else if (significantTokensString.StartsWith("AS "))
                        {
                            ...
                            {
                                KeywordType nextKeywordType = default;
                        ...
                        else
                        {
                            ...
                            KeywordType matchedKeywordType = default;
