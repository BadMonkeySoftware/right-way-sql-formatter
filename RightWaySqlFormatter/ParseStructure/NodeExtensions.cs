...
namespace PoorMansTSqlFormatterLib.ParseStructure
{
    public static class NodeExtensions
    {
        ...
        public static System.Xml.XmlDocument ToXmlDoc(this Node node)
        {
            var doc = new System.Xml.XmlDocument();
            doc.PreserveWhitespace = true;
            doc.AppendChild(BuildXmlElement(doc, node));
            return doc;
        }

        private static System.Xml.XmlElement BuildXmlElement(System.Xml.XmlDocument doc, Node node)
        {
            var el = doc.CreateElement(node.Name);
            if (node.TextValue != null)
                el.AppendChild(doc.CreateTextNode(node.TextValue));
            else if (!node.Children.Any())
                el.AppendChild(doc.CreateTextNode(string.Empty)); // force <Foo></Foo> not <Foo />
            foreach (var attr in node.Attributes)
                el.SetAttribute(attr.Key, attr.Value);
            foreach (var child in node.Children)
                el.AppendChild(BuildXmlElement(doc, child));
            return el;
        }
